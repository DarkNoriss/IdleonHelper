...
export const runBossFarmerLoop = async ({
  token,
  total,
  logPrefix,
}: {
  ...
  logger.log(`${logPrefix}: starting ${total} iterations...`);
  setState("bossFarmer", {
    iteration: 0,
    total,
    running: true,
  ...
  try {
    ...
    logger.log(`${logPrefix}: all ${total} iterations complete.`);
  } finally {
    setState("bossFarmer", {
      iteration: 0,
      total: 0,
