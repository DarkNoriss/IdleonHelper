...
// Verbatim from the game's grimoire self-multiplier conditional.
...
// Verbatim port of the game's `calcGrimoireBonus(upgrades, index)`. Recursion
// terminates because 36 is in SELF_MULTIPLIER_INDICES (depth <= 1).
...
// Verbatim port of the game's grimoire `getUpgradeCost`. DO NOT re-parenthesize.
...
export function getUpgradeCost(
  ...
  const level = levelOf(state, index);
  // Optimizer-side guard - the game returns the formula at any level; we cap
...
// Externals-stripped port of the game's `getWraithStats`. See spec section 4.3
// for reasoning. Per-bone-to-bonus map:
...
export function getWraithStats(state: GrimoireData): GrimoireStats {
  ...
  // The game also multiplies by (1 + bulwarkStyle*total/10000) - dropped.
  ...
  // The game also has wraithForm and marauderStyle factors - dropped.
  const damage =
  ...
  // The game also has marauderStyle - dropped.
  const accuracy =
  ...
  // The game also has bulwarkStyle - dropped.
  const defence =
    (b(2) + b(15) + b(30) + b(40) + b(49)) *
    (1 + (b(7) + b(38)) / 100) *
    (1 + (b(27) * ribcageLog) / 100);

  // crit: base 10 + bonus(10). The game adds famineFishX*lavaLog(1)=0 - dropped.
  const critChance = 10 + b(10);
  // Game formula: 1 + (25 + bonus(20) + famineFishY*lavaLog(1))/100. lavaLog(1)=0.
