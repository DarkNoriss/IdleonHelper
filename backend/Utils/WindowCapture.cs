...
public class WindowCapture
{
    ...
    private static Bitmap CaptureWindowBitmap(IntPtr hwnd, int width, int height)
    {
        ...
        var hBitmap = CreateCompatibleBitmap(_cachedHdcSrc, width, height);
        ...
        var oldBitmap = SelectObject(_cachedHdcDest, hBitmap);

        const int srccopyCaptureblt = 0x00CC0020 | 0x40000000;

        if (!BitBlt(_cachedHdcDest, 0, 0, width, height, _cachedHdcSrc, 0, 0, srccopyCaptureblt))
        {
